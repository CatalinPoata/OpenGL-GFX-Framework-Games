...
void main()
{
    ...
    vec4 color = texture(texture_t, texcoord - vec2(offset.x, offset.z));
    ...
    for(int i = 0; i < 21; i++){
        ...
        float spotlight = dot(-L, vec3(0, -1, -1));
        if(lightTypes[i] == 1){
            ...
            vec3 diffuse_light1 = material_kd / 10 * culoare_lumina * max(dot(norm_world_normal, L1), 0);
            vec3 diffuse_light2 = material_kd / 10 * culoare_lumina * max(dot(norm_world_normal, L2), 0);
            ...
            vec3 specular_light1 = material_ks / 10 * culoare_lumina * primesteLumina1 * pow(max(dot(norm_world_normal, H1), 0), material_shininess);
            vec3 specular_light2 = material_ks / 10 * culoare_lumina * primesteLumina2 * pow(max(dot(norm_world_normal, H2), 0), material_shininess);
            ...
            if(spotlight1 > cos(cutoff)){
                ...
                factorAtenuare1 = pow(linearATT, 2);
            }
            else{
                factorAtenuare1 = 0;
            }
            ...
            if(spotlight2 > cos(cutoff)){
                ...
                factorAtenuare2 = pow(linearATT, 2);
            }
            else{
                factorAtenuare2 = 0;
            }
